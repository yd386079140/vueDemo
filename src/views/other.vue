      ...
      <ol class="demo-box" v-if="todoList.length">
        <li v-for="(item,index) in todoList" :key="index">
          ...
          <a @click="subInfo(index)">-</a>
			...
			<ol class="demo-box" v-else><p class="pWord">什么还没有进行~</p></ol>
      <h2>已经完成
      ...
      <ul v-if="doneList.length">
        <li  v-for="(i,index) in doneList" :key="index">
          ...
          <a @click="subInfomation(index)">-</a>
			...
			<ul v-else><p class="pWord">什么还没有完成~</p></ul>
    </section>
  </div>
</template>
...
.btnCheck{position:absolute;top:2px;left:10px;width:22px;height:22px;cursor:pointer;}
.btnChecked{position:absolute;top:2px;left:10px;width:22px;height:22px;cursor:pointer;}
p{margin: 0;}
...
ol li{cursor:move;}
ul li{border-left: 5px solid #999;opacity: 0.7;}
